...
pub struct Vehicle {
	...
	pub forward_vehicle: VehicleData,
...
impl Vehicle {
	...
	pub fn pull_forward_vehicle(
		&mut self,
		ra_lanes: RwLockReadGuard<Vec<Lane>>,
		ra_vehicles: RwLockReadGuard<Vec<Vehicle>>,
	) {
		self.forward_vehicle = VehicleData::default();
		let ra_active_lane = &ra_lanes[self.active_identity.lane as usize];
		let mut accumulated_distance: f32 = ra_active_lane.length - self.data.distance;
		if let Some(vehicle) = ra_active_lane.vehicles.iter().find(
			|x| {
				let ra_vehicle = &ra_vehicles[**x as usize];
				**x > 0 &&
				ra_vehicle.data.distance > self.data.distance &&
				ra_vehicle.data.identity.vehicle != self.data.identity.vehicle
			}
		) {
			let ra_vehicle = &ra_vehicles[*vehicle as usize];
			let mut vehicle_data = ra_vehicle.data;
			vehicle_data.distance -= self.data.distance;
			self.forward_vehicle = vehicle_data;
			return;
		}
		drop(ra_active_lane);
		self.forward_lanes.iter().find(
			|x| {
				let ra_lane = ra_lanes[x.id as usize];
				let vehicle = ra_lane.vehicles.iter().find(
					|x| {
						let ra_vehicle = &ra_vehicles[**x as usize];
						**x > 0
					}
				...
				match vehicle {
					Some(x) => {
						self.forward_vehicle = ra_vehicles[*x as usize].data;
						true
					},
					None => {
						false
